...
//	if(io.printTVR){}
  ...
  for(int i=0; i<io.N; i++){ MeanTimeEk_n[i] = 0.0; }; time_mean=0;
	...
	g_old.calculateForceFNB(io.eps);		//--  1.3 Calculate force_old with M_old
...
//    cout<< g_old.force[i]<<endl;
  ...
  while (io.ttime < io.total_time){
      ...
      g_new.calculateForceFNB(io.eps);
...
//        g_old.calculateFlux();
      ...
      hb02.RollTheDice();
			...
			g_old.calculateForceFNB(io.eps);		//--  3.3 Calculate force_new with M_new
